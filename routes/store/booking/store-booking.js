...
router.post('/complete', verifySession, async (req, res) => {
  try {
    if (req.body.bookingData) {
      ...
      const booking = await Booking.findByIdAndUpdate(bookingData._id, {
        status: 'completed',
      });
      ...
      if (sum + callDuration >= threshold) {
        booking = await Booking.findByIdAndUpdate(booking._id, {
          status: 'completed',
...
router.post('/bookings', async (req, res) => {
  try {
    ...
    const todayBooking = allBookings
      .filter((booking) => {
        return (
          (booking.booktype === 'booking' &&
...
router.post('/bookings/range', async (req, res) => {
  try {
    ...
    const startDate = new Date(req.body.start).setHours(0, 0, 0, 0);
    const endDate = new Date(req.body.end).setHours(23, 59, 59, 999);
    ...
    const demobookings = await demoBooking
      .find({
        store: store._id,
        startTime: { $gte: startDate, $lte: endDate },
      })
      .select(
        'demoName startTime image description duration demoDate capacity tags customers'
      )
      .sort('startTime');
    ...
    demobookings.forEach(async (demobooking) => {
      ...
      if (
        ...
        if (!archivedemobooking) {
          ...
          demobooking.customers.forEach(async (customer) => {
    ...
    const archivedemobookings = await archiveDemoBooking
      .find({
        store: store._id,
        startTime: { $gte: startDate, $lte: endDate },
      })
...
router.post('/bookings/single', async (req, res) => {
  try {
    ...
    const demobooking = await demoBooking
      .findOne({ _id: bookingData._id })
      .select('demoName description startTime duration channelName image _id');
    demobooking._doc = {
      ...demobooking._doc,
      type: 'live',
      status: 'upcoming',
    ...
    if (!booking && !demobooking) {
      ...
      return res.status(200).json({ booking: booking ? booking : demobooking });
  } catch (error) {
    handleError(error);
...
router.post('/bookings/single/archived', async (req, res) => {
  try {
    ...
    const archivedemobooking = await archiveDemoBooking.findOne({
      _id: bookingData._id,
    });
    archivedemobooking._doc = {
      ...archivedemobooking._doc,
    ...
    return res
      .status(200)
      .json({ booking: booking ? booking : archivedemobooking });
  } catch (error) {
    handleError(error);
    return res.status(500).json({ error: 'Internal server error.' });
  }
