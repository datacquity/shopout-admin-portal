...
router.post('/web/login', async (req, res) => {
  try {
    const { userData, otp, otpDetail } = req.body;
    ...
    await axios.get(
      `https://2factor.in/API/V1/${process.env.TWO_FACTOR_KEY}/SMS/VERIFY/${otpDetail}/${otp}`
    );
    // console.log(userData);

    // Login code
    const foundUser = await User.findOne({ phone: userData.phone }).select(
    ...
    // Signup code
    const savedUser = await createStub(userData);
    ...
    res.status(200).json({
      user: temp,
      newUser: true,
      password: `shopout${userData.phone}`,
    });
  } catch (error) {
    handleError(error);
