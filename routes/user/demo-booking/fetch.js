...
router.post('/archived', async (req, res) => {
  try {
    ...
    const archivedemobookings = await archiveDemoBooking.find({
      youtubeVideo: { $exists: true, $ne: null },
    });

    if (!archivedemobookings) return res.status(400).json({ error: 'archivedemobookings not found.' });
    else return res.status(200).json({ archivedemobookings });
  }
  catch (error) {
    handleError(error);
    return res.status(500).json({ error: 'Internal server error.' });
      ...
      if (!user) return res.status(404).json({ error: 'User not found.' });
        ...
        // if (user.archiveDemoBookings.includes(bookingData._id)) {
          archiveDemoBooking
            ...
            .select(
              'demoName description customers demoDate startTime duration image youtubeVideo'
            ) // add video cloumn for fetch
            .exec((err, archivedemobooking) => {
              let status = 'Upcoming';
              let flag = false;
              var now = timeNow.getTime();
              const startTime = new Date(
                archivedemobooking.startTime
              ).getTime();
              const endTime =
                startTime + (archivedemobooking.duration / 60) * HOUR;
