...
const searchStores = async (query, location, size) => {
  try {
    ...
    ).populate("stores","name location_desc avg_rating business active_hours");
    ...
    await Business.populate(xx, {
      path: 'business',
      select: 'display_name category title_image hasCallNow',
    });
    return xx;
  } catch (e) {
    handleError(e);
...
const searchVideos = async (query, size) => {
    try {
        ...
        if (!searchedTag) {
            searchedTag = await Category.findOne({ name: { $regex: categoryFromName.replace(/ /g, ".") + ".*", $options: 'i' } });
            searchedTag1 = await Category.findOne({ name: { $regex: parentCategoryFromName.replace(/ /g, ".") + ".*", $options: 'i' } });
            console.log(10);
