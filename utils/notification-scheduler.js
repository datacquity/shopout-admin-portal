...
const findDemoBookings = (gt, lt) =>
  new Promise((resolve, reject) => {
    DemoBooking.find({ startTime: { $gte: gt, $lt: lt } })
      .populate('customers.user', '_id firebaseToken deviceToken')
      .populate({
        path: 'business',
...
const sendUpcomingBookingNotification = (booking, type) => {
  ...
  let body = `Your booking with ${storeName} is due at ${formatTime(
    booking.start
  )}. Hope you have an amazing experience.`;
  if (type.localeCompare('booking-15') == 0) body += ` Happy Shopouting.`;
  ...
  dispatchSingleNotificationApple(
    booking.store.deviceToken,
    title,
    `Your booking with a user is due at ${formatTime(
      booking.start
    )}. Happy Selling.`,
    {
      booking: booking._id.toString(),
...
const sendUpcomingBookingNotificationOnlyStore = (booking) => {
  ...
  const storeName = `${booking.store.business.display_name} ${booking.store.name}`;
  const title = 'Upcoming booking';
  const body = `Your booking with a user is due at ${formatTime(
    booking.start
  )}. Happy Selling.`;
...
const sendUpcomingDemoBookingNotification = (demobooking, type) => {
   ...
   * 3. use notification dispatcher to push and save notifications
   */
  console.log('sendUpcomingDemoBookingNotification ==> ', demobooking.demoName);
  const demoName = `${demobooking.demoName}`;
  const title = 'Upcoming Live Event in 60 minutes.';

  let body = `Live demo for ${demoName} starts at ${formatTime(demobooking.startTime)}. Hope it's useful.`;
  ...
  customers.forEach((customer) => {
    dispatchSingleNotification(customer.user.firebaseToken, title, body, {
      demobooking: demobooking._id.toString(),
      archived: 'false',
      type: type,
    });
  ...
  business.stores.forEach((store) => {
    dispatchSingleNotification(
      store.firebaseToken,
      title,
      `Your business has a live event at ${formatTime(demobooking.startTime)}.`,
      {
        demobooking: demobooking._id.toString(),
        archived: 'false',
        type: type,
    ...
    dispatchSingleNotificationApple(
      ...
      title,
      `Your business has a live event at ${formatTime(
        demobooking.startTime
      )}. Happy Selling.`,
      {
        demobooking: demobooking._id.toString(),
        archived: 'false',
    ...
    saveNewNotification(
      title,
      `${demoName} live event starting at ${formatTime(
        demobooking.startTime
      )}.`,
      business.logo,
      null,
...
const sendUpcomingDemoBookingNotificationOnlyUser = (demobooking, type) => {
  console.log(
    'sendUpcomingDemoBookingNotificationOnlyUser ==> ',
    demobooking.demoName
  );

  const demoName = `${demobooking.demoName}`;
  const title = 'Upcoming Live Event in less than a minute.';
...
const sendUpcomingDemoBookingNotificationOnlyStore = (demobooking, type) => {
  ...
  let body = `${demoName} live event starting at ${formatTime(
    demobooking.startTime
  )}.`;
...
const sendCallReminderNotificationOnlyStore = async (booking) => {
  ...
  if (booking.type.localeCompare('virtual') == 0 && temp.length == 0) {
    ...
    const storeName = `${booking.store.business.display_name} ${booking.store.name}`;
    const title = 'Upcoming booking';
    const body = `You forgot the appointment at ${formatTime(
      booking.start
    )}, still possible to connect with Customer. Rush..`;
    dispatchSingleNotification(booking.store.firebaseToken, title, body, {
      booking: booking._id.toString(),
      archived: 'false',
      type: 'booking-1',
    ...
    saveNewNotification(
      title,
      `You forgot the appointment at ${formatTime(
        booking.start
      )}, still possible to connect with Customer. Rush..`,
      booking.store.business.logo,
      null,
...
const findUpcomingBookings = () => {
  ...
  findBookings(gt, lt).then((bookings) => {
    ...
    bookings.forEach((booking) =>
      sendUpcomingBookingNotification(booking, 'booking-15')
    );
  });
};
...
const findUpcomingBookingsShort = () => {
  ...
  findBookings(gt, lt).then((bookings) => {
    console.log(bookings);
    bookings.forEach((booking) =>
      sendUpcomingBookingNotificationOnlyStore(booking)
    );
  });
};
...
const findUpcomingBookingsLong = () => {
  ...
  findBookings(gt, lt).then((bookings) => {
    console.log(bookings);
    bookings.forEach((booking) =>
      sendUpcomingBookingNotification(booking, 'booking-60')
    );
  });
};
...
const findUpcomingBookingsForStore = () => {
  ...
  findBookings(gt, lt).then((bookings) => {
    console.log(bookings);
    bookings.forEach((booking) =>
      sendCallReminderNotificationOnlyStore(booking)
    );
  });
};
...
const findUpcomingDemoBookings = () => {
  const time = new Date().getTime();
  const gt = new Date(time + 1 * HOUR - 25);
  const lt = new Date(time + 1 * HOUR + 25);
  console.log(gt, lt);
  findDemoBookings(gt, lt).then((demobookings) => {
    console.log('demobookings for 60 mins');
    demobookings.forEach((demobooking) =>
      sendUpcomingDemoBookingNotification(demobooking, 'demobooking-60')
    );
  });
};
...
const findUpcomingDemoBookingsShort = () => {
  const time = new Date().getTime();
  const gt = new Date(time + 0.25 * HOUR - 25);
  const lt = new Date(time + 0.25 * HOUR + 25);
  console.log(gt, lt);
  findDemoBookings(gt, lt).then((demobookings) => {
    console.log('demobookings for 15 mins');
    demobookings.forEach((demobooking) =>
      sendUpcomingDemoBookingNotificationOnlyStore(
        demobooking,
        'demobooking-15'
      )
    );
...
const findUpcomingDemoBookingsUserJoin = () => {
  const time = new Date().getTime();
  const gt = new Date(time + 60 * 1000 - 25);
  const lt = new Date(time + 60 * 1000 + 25);
  // console.log(gt, lt);
  findDemoBookings(gt, lt).then((demobookings) => {
    demobookings.forEach((demobooking) =>
      sendUpcomingDemoBookingNotificationOnlyUser(demobooking, 'demobooking-1')
    );
  });
};
  ...
  createArchiveBooking(booking, 'missed').then((archivedId) => {
    if (temp.status.localeCompare('upcoming') === 0) {
      if (temp.type.localeCompare('virtual') == 0 && temper.length == 0) {
        dispatchSingleNotification(
          temp.user.firebaseToken,
          title,
          'We are sorry the store did not call, we will look into it, do you want to check other retailers?',
          {
...
const notificationScript = () => {
  ...
  // Notification scheduler to run at minute 5 and 45.
  schedule.scheduleJob('15,45 * * * *', () => {
    findUpcomingBookings();
    // findUpcomingDemoBookings()
  });

  // Notification scheduler to run at minute 29 and 59.
  ...
  // Notification scheduler to run at minute 5 and 35.
